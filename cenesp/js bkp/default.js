$(function () {
	...
	function formatarTelefone(numero) {
		...
		// Aplicar regex para formatar o número
		if (digits.length == 10)
	...
	if ($("#index").length) {
		...
		$.ajax({ url: apis.novidade + "?shopping_id=" + shopping_id + "&jsoncallback=?", dataType: "json" }).done(function (data) {
			$.each(data, function (i, novidade) {
				...
				if (novidade.novidade_imagem_home_url) {
					imagemNovidade = novidade.novidade_imagem_home_url;
				} else {
					imagemNovidade = apis.upload_site_novidade + novidade.novidade_imagem_home;
				}
				let imagem;
				if (novidade.imagem_url) {
					imagem = novidade.imagem_url;
				} else {
					imagem = novidade.imagem ? apis.upload_site_novidade + novidade.imagem : 'https://upload.madnezz.com.br/7420943d768951630cf61dc319d9d69d';
				}

				$('.novidade_lista').append('<li class="pr novidade_' + novidade.novidade_id + ' ">' +
					'<div><a href="novidade.php?novidade_id=' + novidade.novidade_id + '"><img src="' + imagemNovidade + '" width="100%"></a></div>' +
		...
		load_filme = function (eq, play) {
			...
			//$('.home_cinema>div').html(fundo_trailer+'<h2>'+titulo+'</h2>'/*'<a href="cinema.php?cinema_id='+cinema.id+'"><img src="https://upload.madnezz.com.br/a0064b92495fa9d10db556cb3ae84df9" width="80"/></a -->') */);
			$('.home_cinema>div').html('').append('<div class="home_trailer"><iframe src="' + fundo_trailer + '" frameborder="0" allowfullscreen ></iframe></div><img src="https://upload.madnezz.com.br/a0064b92495fa9d10db556cb3ae84df9" class="play_home" width="80"/><h2>' + titulo + '</h2><a class="to_int" href="' + link + '"></a>');
		...
		$.ajax({ url: apis.cinema_moviecom + "?shopping_id=" + shopping_id + "&jsoncallback=?", dataType: "json" }).done(function (data) {
			$.each(data, function (i, cinema) {
				cinema = cinema.filme;
				iframe = 'src="//www.youtube.com/embed/' + cinema.trailer + '?color=white&iv_load_policy=3&rel=0&showinfo=0&modestbranding=1&autohide=1&autoplay=0" frameborder="0" allowfullscreen';
				$('.home_cinema>ul').prepend(li.replace(/url_trailer/g, iframe).replace(/#filme_url/g, i).replace(/#i/g, i).replace(/#fundo/g, cinema.fundo).replace(/#titulo/g, cinema.titulo).replace(/#genero/g, cinema.genero).replace(/#censura/g, cinema.censura).replace(/#horario/g, cinema.horario));
			});
				...
				$.each(data, function (i, loja) {
					...
					if ($("#loja").length) {
						if (loja.loja_imagem_1_url) {
							...
							imagem = loja.loja_imagem_1 ? apis.upload_site_loja + loja.loja_imagem_1 : 'https://upload.madnezz.com.br/7420943d768951630cf61dc319d9d69d';
						}
					}
					...
					if ($("#alimentacao").length) {
						if (loja.loja_imagem_1_url) {
							...
							imagem = loja.loja_imagem_1 ? apis.upload_site_loja + loja.loja_imagem_1 : 'https://upload.madnezz.com.br/7f5430025dcec818c783aa1fb06bdea2';
						}
					}
					...
					if ($("#servico").length) {
						if (loja.loja_imagem_1_url) {
							...
							imagem = loja.loja_imagem_1 ? apis.upload_site_loja + loja.loja_imagem_1 : 'https://upload.madnezz.com.br/f886a38cf88de5f1e3e746e573dbb445';
							return;
						}
					}
					var texto = loja.loja_texto
						.replace(/WhatsApp: (.*)(\n)?/gi, "<br /><a href='http://api.whatsapp.com/send?phone=55 $1' class='link_whatsapp' target='_blank'><img src='https://upload.madnezz.com.br/20f63cf636d695d27c43c582ef28144b' alt='WhatsApp' class='icon' /> $1</a>")
						.replace(/Delivery: (.*)(\n)?/gi, "<br /><a href='$1' target='_blank'><img src='https://upload.madnezz.com.br/8d53b932a9f526a64f52191e692a01d1' alt='Delivery' class='icon' /></a>")
						.replace(/iFood/g, "<br /><img src='https://upload.madnezz.com.br/3f3f9d9fb24dbd07684de1ee4c73773a' alt='iFood' class='icon' /> iFood")
						.replace(/Rappi/g, "<br /><img src='https://upload.madnezz.com.br/050a2460a9ba80c273e36042165256e8' alt='Rappi' class='icon' /> Rappi")
						.replace(/Telefone:/g, "<br /><img src='https://upload.madnezz.com.br/4164c61000b7ed50582116fa17244939' alt='Telefone' class='icon' />")
						.replace(/Tel:/g, "<br /><img src='https://upload.madnezz.com.br/4164c61000b7ed50582116fa17244939' alt='Telefone' class='icon' />")
						.replace(/Atendimento a domicílio:/gi, "<span>Atendimento a domicílio:</span><br />");

					if (loja.instagram) {
						texto += "<br /><a href='" + loja.instagram + "' target='_blank'><img src='https://upload.madnezz.com.br/a567fb75254cca4400910eeac294cd63' alt='Instagram' class='icon' /> Instagram</a>";
					}

					$('.loja_lista').append('<li class="pr loja_' + loja.id + ' " value="' + loja.id + '">' +
						'<img src="' + imagem + '"  />' +
						'<h2 class="ls2" >' + loja.loja_nome + '</h2><br>' +
						'<div class="info-lojas">' +

						'<p><span>Segmento:</span> ' + loja.ramo_nome + '</p>' +
						(loja.loja_telefone ? '<p><span>Telefone:</span> ' + formatarTelefone(loja.loja_telefone) + '</p>' : '') +
						(loja.whatsapp ? `
							<p>
								<span></span>
									<a class="whasapp_ancor" style="color: #31636c;" href="https://wa.me/55` + limparTelefone(loja.whatsapp) + `" target="_blank">
										<img src="https://upload.madnezz.com.br/20f63cf636d695d27c43c582ef28144b" alt="Icone Whatsapp" class="loja_whatsapp" />
		...
		$.ajax({ url: apis.loja + "?tipo=" + tipo + "&shopping_id=" + shopping_id + "&full=true&llj=true&jsoncallback=?", dataType: "json" }).done(function (data) {
			//Monta os ramos
		...
		$(".bt_busca").click(function () {
			if ($(".filtro_nome").val()) {
				...
				$.ajax({ url: apis.loja + "?tipo=" + tipo + "&shopping_id=" + shopping_id + "&filtro_nome=" + filtroNome + "&full=true&jsoncallback=?", dataType: "json" }).done(function (data) {
					monta_loja(data);
				});
			}
		});
		...
		$('.alfabeto li').click(function (e) {
			_gaq.push(["_trackPageview", "loja.php?filtro_letra=" + $(this).text()]);
			$('.alfabeto li').removeClass('active');
			$(this).addClass('active');
			$(".filtro_ramo").val("");
			$(".filtro_nome").val("");
			$(".loja_lista").html("");
			$.ajax({ url: apis.loja + "?tipo=" + tipo + "&shopping_id=" + shopping_id + "&filtro_letra=" + $(this).text() + "&full=true&jsoncallback=?", dataType: "json" }).done(function (data) {
				monta_loja(data);
			});
		});
		...
		function monta_novidade(data) {
			...
			if (data.length) {
				...
				$.each(data, function (i, novidade) {
					...
					let imagemHome;
					if (novidade_novidade_imagem_home_url) {
					...
					if (novidade.novidade_imagem_1_url) {
						...
						imagemNovidade = apis.upload_site_novidade + novidade.novidade_imagem_1;
					}

					$('.novidade_lista').append('<li class="pr" data-id="' + novidade.novidade_id + '" ">' +
						'<div class="img-close"><img src="' + imagemHome + '"  /></div>' +
						'<div class="img-open"><img src="' + imagemNovidade + '"  /></div>' +
						'<div class="info-novidade">' +
						...
						'<div class="fb-share-button" data-href="' + monta_sharer(novidade.novidade_id, conteudo, share_img) + '" data-layout="button" data-size="large" data-mobile-iframe="false"><a class="fb-xfbml-parse-ignore" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=' + monta_sharer(novidade.novidade_id, conteudo, share_img) + '"></a></div>' +
						'<a href="whatsapp://send?text=' + novidade.novidade_nome + ': https://shoppingjaragua.com.br/cenesp/novidade.php?novidade_id=' + novidade.novidade_id + '" class="whatsapp"></a>' +
				...
				$.each(data, function (i) {
					...
					$('.vagas_lista').prepend(`
						<li class="vagas">
							...
							<button id="`+ this.id + `">Candidatar-se</button>
						</li>
					`);
				});
			} else {
		...
		$('#oportunidade_form').submit(function (event) {
			...
			if (!$("#oportunidade_form input[name=nome] ").val() || !$("#oportunidade_form input[name=email] ").val() || !$("#oportunidade_form input[name=cpf] ").val() || !$("#oportunidade_form input[name=anexo] ").val()) {
				alert('Favor preencher todos os campos');
				return false;
			}
		});

		$('.vagas_lista').on('click', 'li button', function () {
			loja = $(this).parents('li').find('h2').text();
			vaga = $(this).parents('li').find('h3').text();
			$('#oportunidade').find('[name=area]').val(loja + ' - ' + vaga);

			var id = $(this).attr('id');
			var loja = $(this).parents('li').find('h2').text();
			if ($('[name="loja"]').length <= 0) {
				$('<td colspan="2">' +
					'<div class="position-relative">' +
					'<input type="text" name="loja" class="wd100" value="Loja ' + loja + '" readonly />' +
					'<i class="fas fa-times close-alert close-loja"></i>' +
					'</div>' +
					'</div>').appendTo($('[name="nome"]').parents('tr').prev('tr'));
			}
			$('[name="vaga"], [name="vaga_id"]').val(id);
			$('[name="tp"]').val(1);
			$('[name="area"]').parents('td').hide();

			$('html, body').animate({
				scrollTop: $('#oportunidade_form').offset().top - 350
			}, 500);
		});
