			...
			if(data.length){ 
				$.each(data,function(i,novidade){
					if (i<=2){
						...
						console.log(dataEntrada[1]+' '+dataEntrada[0]);
						$('.novidade_lista').append('<li id="'+dataEntrada[2]+dataEntrada[1]+dataEntrada[0]+'" class="pr" data-id="'+novidade.novidade_id+'" ">'+
							'<div class="img-close"><a href="/indaiatuba/novidade.php?novidade_id='+this.novidade_id+'" target="_new">'+
								'<img src="'+apis.upload_site_novidade+novidade.novidade_imagem_home+'"  />'+
							'</a></div>'+
							'<div class="info-novidade">'+
								'<h3 class="green2 mt0">'+(novidade.novidade_nome.length>50?novidade.novidade_nome.substr(0,novidade.novidade_nome.indexOf(' ',23))+'...':novidade.novidade_nome).replace(/<br><br>/g,'').replace(/<br \/><br \/>/g,'<br>')+'</h3><br>'+
